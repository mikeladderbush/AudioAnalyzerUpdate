...
int decodeMp3(char *mp3file)
{
    ...
    while (!mp3fileStream.eof())
    {
        ...
        {
            // TODO: Process the frame data here
            ...
            std::bitset<2> extractedMpegVersion((syncbits.to_ulong() >> 19) & 0b10);

            // The Layer description which is 2,
            std::bitset<2> extractedLayerDescription((syncbits.to_ulong() >> 17) & 0b10);
            ...
            std::bitset<4> extractedBitrateIndex((syncbits.to_ulong() >> 12) & 0b1010);

            // Here I am going to create a separate variable that concatenates previous variables in order to better find our bitrate.
            std::bitset<8> bitrateIndexValue = (((syncbits.to_ulong() >> 12) & 0b1010) << ((syncbits.to_ulong() >> 17) & 0b10) << ((syncbits.to_ulong() >> 19) & 0b10));

            // Then we have 2 bits for the frequency index which we should also create a table for.
            std::bitset<2> extractedFrequencyIndex((syncbits.to_ulong() >> 10) & 0b10);
            ...
            std::bitset<2> extractedChannelMode((syncbits.to_ulong() >> 6) & 0b10);

            // The next 2 is the Mode extension.
            std::bitset<2> extractedModeExtension((syncbits.to_ulong() >> 4) & 0b10);
            ...
            std::bitset<2> extractedEmphasisBits((syncbits.to_ulong() >> 0) & 0b10);
...
int main()
{
    ...
    file.close();
    // make a list of different aspects of the data
    // make a function that takes the file in and breaks it down by sample rate
    // load  the samples into a buffer
    // analyze the buffer and utilize the FFT to find the frequency
    return 0;
}
